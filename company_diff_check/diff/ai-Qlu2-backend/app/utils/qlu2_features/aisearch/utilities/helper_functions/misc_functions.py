...
def extract_marker(response, marker="</reasoning>", position="after"):
    """
    Extracts the text after or before the given marker in the response string.
    If position is 'after', returns the substring after the marker.
    If position is 'before', returns the substring before the marker.
    If marker not found, returns the original response.
    """
...
def extract_and_format_names(xml_string):
    """
    Extracts industry names from an XML-like string and returns them
    as a formatted bullet list string.

    Args:
        xml_string: A string containing the XML-like data.

    Returns:
        A string formatted as a markdown bullet list.
    """
    try:
        # This regular expression finds all text enclosed by the <target_industry_name> tags.
        # The (.*?) part is a non-greedy capture group for any characters,
        # and re.DOTALL ensures it works even if the content spans multiple lines.
        pattern = re.compile(
            r"<target_industry_name>(.*?)</target_industry_name>", re.DOTALL
        )

        # Find all matches in the input string
        matches = pattern.findall(xml_string)

        # Clean up any leading/trailing whitespace from the extracted names
        # and format them into a bulleted list.
        if not matches:
            return "No target industry names were found."

        bullet_points = [f"* {match.strip()}" for match in matches]

        return "\n".join(bullet_points)
    except Exception as e:
        print(f"Error extracting and formatting names: {e}")
        return ""
