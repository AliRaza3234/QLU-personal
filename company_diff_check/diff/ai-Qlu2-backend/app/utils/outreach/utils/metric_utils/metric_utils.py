...
from app.utils.outreach.utils.metric_utils.metric_utils_prompts import (
...
async def getToolTip(intent: str, metric: str):
    systemPrompt = deepcopy(TOOLTIP_SYSTEM_PROPMT)
    userPrompt = deepcopy(TOOLTIP_USER_PROPMT)

    userPrompt = userPrompt.format(intent=intent, metric=metric)
    ...
    res = await gpt_runner(chat, 0.2, "gpt-4o-mini", True)
    res = json.loads(res)
    res = res["definition"]
...
def thresholdValidate(threshold):
    threshold = (threshold / 100) * 35
    return threshold if threshold % 5 == 0 else round((threshold / 5)) * 5
